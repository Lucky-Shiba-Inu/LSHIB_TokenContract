...
//
// Copyright of The LUCKYSHIBAINU Team
// _    _   _  ____ _  ____   ______  _   _ ___ ____    _    ___ _   _ _   _
//| |  | | | |/ ___| |/ /\ \ / / ___|| | | |_ _| __ )  / \  |_ _| \ | | | | |
//| |  | | | | |   | ' /  \ V /\___ \| |_| || ||  _ \ / _ \  | ||  \| | | | |
//| |__| |_| | |___| . \   | |  ___) |  _  || || |_) / ___ \ | || |\  | |_| |
//|_____\___/ \____|_|\_\  |_| |____/|_| |_|___|____/_/   \_\___|_| \_|\___/
//
// There is an automated LuckyDraw alongside every LUCKYSHIBAINU buy transaction
// 
// LUCKYSHIBAINU has 9% tax split across 7 awesome protocols
// (Sells incur 16% tax and DumpSells incur 23%)
// 
// 3%: LuckyDraw
// 1%: Burn
// 1%: Buyback
...
//IPinkAntiBot Interface from https://github.com/pinkmoonfinance/pink-antibot-guide/blob/main/IPinkAntiBot.sol
...
/**
 * @dev The official LuckyShibaInu smart contract
 */
contract LuckyShibaInu is Context, IERC20, Ownable {
    // PinkBOT initialization
    IPinkAntiBot public pinkAntiBot;
    //switch to turn Anti-bot or not
    ...
    // General Info
    string private _name     = "LuckyShibaInu";
    string private _symbol   = "LSHIB";
    uint8  private _decimals = 9;
    
    // Liquidity Settings
    IUniswapV2Router02 public _pancakeswapV2Router; // The address of the PancakeSwap V2 Router
    address public _pancakeswapV2LiquidityPair;     // The address of the PancakeSwap V2 liquidity pairing for LSHIB/WBNB
    ...
    // Addresses
    address payable public _marketingAddress = payable(0xdC615dD001fF3197935A3B0f1806571f9233af25); // Marketing address used to pay for marketing
    ...
    // Token Tax Settings
    uint256 public  _taxFee           = 9;                    // 9% tax 
    uint256 public  _sellTaxFee       = 16;                   // 16% tax 
    uint256 public  _whaleSellTaxFee  = 23;                   // 23% tax 
    uint256 private _previousTaxFee;
    uint256 public  _lotteryPortionMultiply = 3;              // 3% lottery. how many parts of tFee Tokens to go to Lottery
    ...
    uint256 public _buybackBNBThreshold  = 1 * 10**17;        // 0.1 BNB will be required to be inside the contract before a buyback will be performed
    uint256 public _buybackUpperLimit    = 1 * 10**9 * 10**9; // 1 BNB will be used to buy at maximum in any one trade
    uint256 public _buybackBNBPercentage = 100;               // 100% of BNB is used to buyback tokens
    ...
    uint    public  _lotteryChance       = 50;                 // 5% chance to win on a buy
    uint256 public  _lotteryThreshold    = 10 * 10**1 * 10**9; // 100 tokens required to be in the pool before lottery can be triggered
    uint256 public  _lotteryMinimumSpend = 20 * 10**9 * 10**9; // 20 billion tokens required to buy before before lottery can be triggered
    address public  _previousWinner;
    uint256 public  _previousWonAmount;
    uint    public  _previousWinTime;
    uint    public  _lastRoll;
    uint256 private _nonce;
    uint256 public  _totalLotteryWon;  // Total Lottery Won or given away so far
    ...
    constructor (address pinkAntiBot_) {
        ...
        // Pink AntiBOT Enabled by default
        antiBotEnabled = true;
    ...
    /**
     * @notice Withdraws non-LUCKYSHIBAINU tokens that are stuck as to not interfere with the liquidity
     */
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        // Gets the contracts LUCKYSHIBAINU balance for buybacks, liquidity and marketing
    ...
    /**
     * @notice Burns LUCKYSHIBAINU tokens straight to the burn address
     */
    ...
    function swapAndLiquify(uint256 tokenAmount) private lockSwapping {
        ...
        // Swap 7/8ths of LUCKYSHIBAINU tokens for BNB
        ...
        // We now have 6/7ths left of BNB (converted from LUCKYSHIBAINU)
    ...
    function swapTokensForBNB(uint256 tokenAmount) private {
        // Generate the Pancakeswap pair for LSHIB/WBNB
    ...
    function swapBNBForTokens(uint256 amount) private {
        ...
        _pancakeswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: amount}(
            0, // Accept any amount of LUCKYSHIBAINU
            path,
